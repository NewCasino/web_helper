...
DECLARE @START_DATE NVARCHAR(50)  
DECLARE @SITE1 NVARCHAR(50) 
DECLARE @SITE2 NVARCHAR(50) 
SET @START_DATE='2014-11-15 00:00:00'
SET @SITE1='marathonbet'
SET @SITE2='gobetgo'
...
(
	...
	from  europe_100_log 
	where f_start_time>@START_DATE  and timespan >@START_DATE and website=@SITE1
) a
union all
select count(*) from
(
	select distinct website,start_time,host,client,odd_win,odd_draw,odd_lose
	from  europe_100_log 
	where f_start_time>@START_DATE and timespan >@START_DATE and website=@SITE2
) b
...
from  europe_100_log
where f_start_time>@START_DATE and website in (@SITE1,@SITE2)
