...
class AnalyseHelper
{
    public static bool is_same_event(string start_time1, string input_home1, string input_away1, string start_time2, string input_home2, string input_away2)
    {
        string home1 = clear_team_name(input_home1);
        string away1 = clear_team_name(input_away1);
        string home2 = clear_team_name(input_home2);
        string away2 = clear_team_name(input_away2);

        DateTime dt1 = Tool.get_time(start_time1);
        DateTime dt2 = Tool.get_time(start_time2);
        TimeSpan span = dt2 - dt1;

        if (span.TotalHours > -48 && span.TotalHours < 48 && is_alike_name(home1, home2) && is_alike_name(away1, away2)) return true;
        if (span.TotalHours > -48 && span.TotalHours < 48 && (is_alike_name(input_home1, input_home2) || is_alike_name(input_away1, input_away2))) return true;
        return false;
    }
    ...
    {
        if (Match100Helper.convert_team_name(input1) == Match100Helper.convert_team_name(input2)) return true;
        return false;
    }
