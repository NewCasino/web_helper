...
class MbookSQL
{
    ...
    {
        string sql = "";
        sql = "delete from s_mbook_market where event_id='{0}' and market='{1}' and runner='{2}' and depth_no>='{3}' and type='{4}'";
        sql = string.Format(sql, event_id, market_name, runner_name, depth_no,type);
        SQLServerHelper.exe_sql(sql);

        sql = " insert into s_mbook_market (timespan,event_id,market,runner,depth_no,type,odd,amount) values" +
            " ( '{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}')";
