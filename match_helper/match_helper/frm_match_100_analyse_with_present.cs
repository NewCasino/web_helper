...
namespace match_helper
{
    public partial class frm_match_100_analyse_with_present : Form
    {
        ...
        private void btn_compute_2_Click(object sender, EventArgs e)
        {
            ArrayList list_websites = new ArrayList();
            foreach (DataGridViewRow row in dgv_website.Rows)
            {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                {
                    BsonDocument doc = Match100Analyse.get_max_from_two_match_with_persent_2(dt.Rows[i]["start_time"].ToString(),
                                                                              dt.Rows[i]["host"].ToString(),
                                                                              dt.Rows[i]["client"].ToString(),
            ...
            if (cb_two_persent_asc_2.Checked) get_two_by_persent_asc(list);
            if (cb_two_persent_desc_2.Checked) get_two_by_persent_desc(list); 
            if (cb_two_start_time_asc_2.Checked) get_two_by_start_time_asc(list);
            if (cb_two_start_time_desc_2.Checked) get_two_by_start_time_desc(list);
        }
