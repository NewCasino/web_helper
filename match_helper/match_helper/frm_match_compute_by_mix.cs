    ...
    public partial class frm_match_compute_by_mix : Form
    {
        ...
        private void btn_1_min_1_wdl_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dgv_match.Rows)
            {
                if (Convert.ToBoolean(row.Cells["selected"].Value) == true)
                {
                    ...
                    BsonDocument match = MatchMix.get_mix_doc_from_db(id);
        ...
        private void btn_2_min_2_wld_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                {
                    BsonDocument match1 = MatchMix.get_mix_doc_from_db(list[i].ToString());
                    BsonDocument match2 = MatchMix.get_mix_doc_from_db(list[j].ToString());
                    for (int k = 1; k <= 50; k++)
                    {
